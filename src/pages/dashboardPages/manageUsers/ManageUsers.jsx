...
import { AudioOutlined, CheckCircleOutlined, IssuesCloseOutlined } from '@ant-design/icons';
const { Search } = Input;
import { Button, Modal } from 'antd';
import Swal from 'sweetalert2';
import { useDeleteUserMutation, useGetUsersQuery, useStatusChangeUserMutation } from '../../../redux/dashboardFeatures/manageUsers/dashboardManageUsersApi';
...
const ManageUsers = () => {
  ...
  const handleToggle = async (id) => {
    try {
      const res = await statusChangeUser(id).unwrap()
      console.log(res)
      if (res.status === true) {
        toast.success(res?.message)
        await refetch()
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="flex items-center justify-end gap-3">
          {
            record.verified_status === 'verified' && <div className="relative group w-fit">
